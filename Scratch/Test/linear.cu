...
int main() {
    ...
    // out_cpu and out_gpu are placeholders, and their content will be replaced
    // We don't need to change their values
...
/*
    // The bias are set to zero in both cases,
    // so no need to worry about them
    // However, the weights are randomly initialized with Kaiming initialization
    ...
    // test_res prints the sum of the element-wise difference between the two vectors
